...
clear
INTERACTIVE=true
usage() { echo ;; echo "Testing and installation script. Options: -i true or false for interaction." 1>&2; exit 1; }
...
# get almost collision_set out of small test set as it doesn"t contain any depth images
# rm small/test_set.txt
echo
...
test_in_docker_container(){
	if [ $INTERACTIVE = true ] ; then	
		read -p "$(tput setaf 2) continue? ([y]/n) $(tput sgr 0)" answer
		answer="_$answer"
		if [ $answer == '_n' ] ; then
			exit
		fi
...
echo "Then it is time to start with step_two which you can find [here](https://github.com/kkelchte/doshico/tree/master/assets/instructions)"
echo "Goodluck!"
